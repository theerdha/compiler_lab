                ...
                | declarator '=' initializer 
                  {
                  	...
                  	else
                  	{
						...
						*/
                  		use = "t"+conv2string(x - 2);
                  		//maa = "m[0]";
                  		quadArr.emit(OP_mateq,"0",use,$1->name);

                  		use = "t"+conv2string(x - 1);
                  		//maa = "m[4]";
                  		quadArr.emit(OP_mateq,"4",use,$1 -> name);

                  		for(int  i = 0; i < mat_init.size(); i++)
                  		{
                  			use = "t"+conv2string(x+i);
                  			//maa = "m[" + conv2string(i * 8 + 8) + "]";
                  			quadArr.emit(OP_mateq,conv2string(i * 8 + 8),use,$1 -> name);
                  		}
			        ...
			        | postfix_expression '(' argument_expression_list ')' 
			          {
			          	...
			          	symbol_entry * a = global_symbol_table -> lookup($1);
			          	$1 -> nested = a -> nested;
