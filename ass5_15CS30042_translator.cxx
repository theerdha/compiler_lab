...
void func_utility::method(symbol_entry* &s)
{
	...
	while(j <= i)
	{
		quadArr.emit(OP_par,(l.at(j)) -> name);
		j++;
	}
	...
	while(j <= i)
	{
		se = (temp -> entries).at(j);
		cout << se -> name << endl;
		se = updatein(se,((l.at(j)) ->initial).int_initial);
		j++;
	}*/
	

	symbol_entry* t1 = temp -> gentemp(temp->f->returntype);
...
void symbol_entry::print()
{
	...
	if(entry_type.bt == int_type) outfile << "int\t\t\t\t\t";
	if(entry_type.bt == double_type) outfile << "double\t\t\t\t";
	if(entry_type.bt == void_type) outfile << "void\t\t\t\t";
	if(entry_type.bt == char_type) outfile << "char\t\t\t\t";
	if(entry_type.bt == matrix_type) outfile << "matrix\t\t\t\t";
	if(entry_type.bt == bool_type) outfile << "bool\t\t\t\t\t";
	if(entry_type.bt == function_type) outfile << "function\t\t\t";
	if(entry_type.bt == pointer_type) outfile << "pointer\t\t\t\t";

	if(is_initialized)
	{

		if(entry_type.bt == int_type) outfile << initial.int_initial << "\t\t\t\t\t";
		else if(entry_type.bt == double_type) outfile << initial.double_initial << "\t\t\t\t\t";
		else if(entry_type.bt == char_type) outfile << initial.char_initial << "\t\t\t\t\t";
	...
	if(nested != NULL) outfile << nested -> name;
	else outfile << "null";
...
void symbolTable::print()
{
	...
	for(int i=0; i < entries.size(); i++)
	{
		if(entries.at(i) -> nested != NULL) 
		{
			...
			entries.at(i) -> nested -> updatesizes();
			entries.at(i) -> nested -> print();
			entries.at(i) -> nested -> ComputeOffset();
		}
	}

}

void symbolTable::ComputeOffset()
{
	this -> offset = entries.at(entries.size()-1) -> offset;
}

int symbolTable::getOffset(string s)
{
	for(int i=0; i < entries.size(); i++)
	{
		if((entries.at(i)) -> name.compare(s) == 0)
		{
			return (entries.at(i)) -> offset;
		}
	}
}

type symbolTable::getType(string s)
{
	for(int i=0; i < entries.size(); i++)
	{
		if((entries.at(i)) -> name.compare(s) == 0)
		{
			return (entries.at(i)) -> entry_type;
		}
	}
}

void symbolTable::updatesizes()
{
	for(int i=0; i < entries.size(); i++)
	{
		if((entries.at(i) -> entry_type).bt == int_type) entries.at(i) -> size = 4;
		if((entries.at(i) -> entry_type).bt == double_type) entries.at(i) -> size = 8;
		if((entries.at(i) -> entry_type).bt == char_type) entries.at(i) -> size = 1;
		if((entries.at(i) -> entry_type).bt == void_type) entries.at(i) -> size = 0;
		if((entries.at(i) -> entry_type).bt == pointer_type) entries.at(i) -> size = 4;
	}

	entries.at(0) -> offset = 0;

	for(int i=1; i < entries.size(); i++)
	{
		if((entries.at(i) -> entry_type).bt == double_type)
		{
			entries.at(i) -> offset = entries.at(i-1) -> offset + entries.at(i-1) -> size;
			if((entries.at(i) -> offset) % 8 != 0)entries.at(i) -> offset = (entries.at(i) -> offset/8 + 1)*8;
		}

		if((entries.at(i) -> entry_type).bt == int_type)
		{
			entries.at(i) -> offset = entries.at(i-1) -> offset + entries.at(i-1) -> size;
			if((entries.at(i) -> offset) % 4 != 0)entries.at(i) -> offset = (entries.at(i) -> offset/4 + 1)*4;
		}
	}
}
...
{
	quad_entry * q;
	outfile.open("ass5_15CS30042_quads5.out");
	for(int i = 0; i < arr.size(); i++)
	{
		q = arr.at(i);
		opcodeType op = q -> op;
		if(op == OP_add) outfile << i << "\t\t\t" << q -> result <<" = "<< q->arg1<<" + "<< q -> arg2 << endl;
		else if(op == OP_sub) outfile << i << "\t\t\t" << q -> result <<" = "<<q -> arg1<<" - "<< q -> arg2 << endl;
		else if(op == OP_mul) outfile << i << "\t\t\t" << q -> result <<" = "<<q -> arg1<<" * "<< q -> arg2 << endl;
		else if(op == OP_div) outfile << i << "\t\t\t" << q -> result <<" = "<<q -> arg1<<" / "<< q -> arg2 << endl;
		else if(op == OP_mod) outfile << i << "\t\t\t" << q -> result <<" = "<<q -> arg1<<" % "<< q -> arg2 << endl;
		else if(op == OP_shl) outfile << i << "\t\t\t" << q -> result <<" = "<<q -> arg1<<" << "<< q -> arg2 << endl;
		else if(op == OP_shr) outfile << i << "\t\t\t" << q -> result <<" = "<<q -> arg1<<" >> "<< q -> arg2 << endl;
		else if(op == OP_lt) outfile << i << "\t\t\t" << q -> result <<" = "<<q -> arg1<<" < "<< q -> arg2 << endl;
		else if(op == OP_lte) outfile << i << "\t\t\t" << q -> result <<" = "<<q -> arg1<<" <= "<< q -> arg2 << endl;
		else if(op == OP_gt) outfile << i << "\t\t\t" << q -> result <<" = "<<q -> arg1<<" > "<< q -> arg2 << endl;
		else if(op == OP_gte) outfile << i << "\t\t\t" << q -> result <<" = "<<q -> arg1<<" >= "<< q -> arg2 << endl;
		else if(op == OP_and) outfile << i << "\t\t\t" << q -> result <<" = "<<q -> arg1<<" && "<< q -> arg2 << endl;
		else if(op == OP_or) outfile << i << "\t\t\t" << q -> result <<" = "<<q -> arg1<<" || "<< q -> arg2 << endl;
		else if(op == OP_logeq) outfile << i << "\t\t\t" << q -> result <<" = "<<q -> arg1<<" == "<< q -> arg2 << endl;
		else if(op == OP_neq) outfile << i << "\t\t\t" << q -> result <<" = "<<q -> arg1<<" != "<< q -> arg2 << endl;
		else if(op == OP_bitxor) outfile << i << "\t\t\t" << q -> result <<" = "<<q -> arg1<<" ^ "<< q -> arg2 << endl;
		else if(op == OP_bitand) outfile << i << "\t\t\t" << q -> result <<" = "<<q -> arg1<<" & "<< q -> arg2 << endl;
		else if(op == OP_bitor) outfile << i << "\t\t\t" << q -> result <<" = "<<q -> arg1<<" | "<< q -> arg2 << endl;
		else if(op == OP_uminus) outfile << i << "\t\t\t" << q -> result <<" =  - "<< q -> arg1 << endl;
		else if(op == Int2Double) {}//outfile << i << "\t\t\t"<<q -> result<<" = Int2Double "<<q -> arg1<<endl;
		else if(op == Double2Int) {}//outfile << i << "\t\t\t"<<q -> result<<" = Double2Int "<<q -> arg1<<endl;
		else if(op == Char2Int) {}//outfile << i << "\t\t\t"<<q -> result<<" = char2Int "<<q -> arg1<<endl;
		else if(op == Int2Char) {}//outfile << i << "\t\t\t"<<q -> result<<" = Int2Char "<<q -> arg1<<endl;
		else if(op == OP_eq) outfile << i << "\t\t\t"<<q -> result <<" = "<< q -> arg1 <<endl;
		else if(op == OP_IFFalseGOTO) outfile << i << "\t\t\t"<< "IFFALSE "<< q -> arg1<<" GOTO "<< q -> result <<endl;
		else if(op == OP_IFLessGOTO) outfile << i << "\t\t\t"<<"IF "<< q -> arg1 <<" < "<< q -> arg2 <<" GOTO "<< q -> result <<endl;
		else if(op == OP_IFGrtGOTO) outfile << i << "\t\t\t"<<"IF "<< q -> arg1 <<" > "<< q -> arg2 <<" GOTO "<< q -> result <<endl;
		else if(op == OP_IFLessEqGOTO) outfile << i << "\t\t\t"<<"IF "<< q -> arg1 <<" <= "<< q -> arg2 <<" GOTO "<< q -> result <<endl;
		else if(op == OP_IFGrtEqGOTO) outfile << i << "\t\t\t"<<"IF "<< q -> arg1 <<" >= "<< q -> arg2 <<" GOTO "<< q -> result <<endl;
		else if(op == OP_IFNotEqGOTO) outfile << i << "\t\t\t"<<"IF "<< q -> arg1 <<" != "<< q -> arg2 <<" GOTO "<< q -> result <<endl;
		else if(op == OP_IFLogEqGOTO) outfile << i << "\t\t\t"<<"IF "<< q -> arg1 <<" == "<< q -> arg2 <<" GOTO "<< q -> result <<endl;	
		else if(op == OP_call)	outfile << i << "\t\t\t" <<q -> result <<" = CALL "<< q -> arg1 <<", "<< q -> arg2 <<endl;
		else if(op == OP_GOTO) outfile << i << "\t\t\t"<<"GOTO "<< q -> result <<endl;
		else if(op == OP_par) outfile << i << "\t\t\t"<<"PARAM "<< q -> result <<endl;
		else if(op == OP_ret) outfile << i << "\t\t\t"<<"RET "<< q -> result <<endl;
		else if(op == OP_mateq) outfile << i << "\t\t\t"<< q -> result <<"["<< q -> arg1 <<"]"<<" = "<< q -> arg2 <<endl;
		else if(op == OP_eqmat) outfile << i << "\t\t\t" << q -> result <<" = "<< q -> arg1 <<"["<< q -> arg2 << "]" <<endl;
		else if(op == OP_addr) outfile << i << "\t\t\t" << q -> result <<" = &"<< q -> arg1 <<endl;
		else if(op == OP_LAB) outfile << i << "\t\t\t" << "LABEL " << q -> result <<endl;
		else if(op == OP_transpose) outfile << i << "\t\t\t" << q -> result << " = " << q -> arg1 << ".'"<<endl;
